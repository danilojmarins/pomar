...
export default class UpdateHarvestUseCase {
    ...
    async execute(information: string, date: string, weight: number, tree_id: string, id: string): Promise<void> {
        const tree = await this._treeGateway.findById(tree_id);
        if (!tree) {
            throw new Error('tree does not exists');
        }
        const harvest = new Harvest(information, date, weight, tree, id);
        await this._harvestGateway.update(harvest);
    }
}